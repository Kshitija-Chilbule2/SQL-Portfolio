...
SELECT u.user_id, u.name, COUNT(w.user_id) AS Total_Workouts
FROM users u
JOIN workout_sessions w ON u.user_id = w.user_id
...
SELECT MIN(duration_minutes) AS shortest_workout_duration, MAX(duration_minutes) AS longest_workout_duration
FROM workout_sessions;
...
WHERE e.name = 'Squat'
GROUP BY User_Name, Exercise_Name;
